...
export function switchView(view) {
    ...
    if (view === 'targets') {
        ...
        if (picker && !picker.value) picker.value = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
        if(window.loadTargetView) window.loadTargetView();
    }
}

export function switchChartTab(tab) {
    ['chart-tab-overview', 'chart-tab-detail', 'chart-tab-target_prog'].forEach(t => { document.getElementById(t).classList.add('hidden'); });
    ['btn-tab-overview', 'btn-tab-detail', 'btn-tab-target_prog'].forEach(b => { document.getElementById(b).classList.remove('active'); });
...
export function toggleSidebar() { document.getElementById('sidebarMobile').classList.toggle('-translate-x-full'); }
export function ui_showMsg(txt, color) { const el = document.getElementById('msg'); if(el) { el.className = `text-center text-xs font-bold mt-4 h-5 text-${color}-500`; el.innerText = txt; } }
...
export function ui_updateSVNOptions() {
    ...
    if (selProv) {
        ...
        const shopOpts = relevantShops.map(s => `<option value="${s.shop_code}">${STATE.submittedShops.has(s.shop_code) ? "✅ " : ""}${s.shop_code} - ${s.shop_name}</option>`).join('');
...
export function ui_updateShopInfo() {
    ...
    if (!shopCodeEl || !shopCodeEl.value) { if(disp) disp.classList.add('hidden'); return; }
...
export function ui_renderModelOptionsAll() {
    document.querySelectorAll('.model-select').forEach(sel => { const curr = sel.value; sel.innerHTML = getModelOptionsHtml(curr); sel.value = curr; });
}

export function ui_addSaleRow(data = {}) {
    const tr = document.createElement('tr');
    tr.className = "bg-white border-b border-gray-100 hover:bg-gray-50 transition";
    tr.innerHTML = `<td class="p-2"><select class="model-select w-full bg-white border border-gray-200 rounded p-1.5 text-xs font-bold outline-none" onchange="onModelChange(this)">${getModelOptionsHtml(data.model)}</select></td><td class="p-2 border-l border-orange-100"><input type="text" class="qty-si-input w-full text-center border border-orange-200 rounded p-1.5 text-xs font-bold text-orange-700 bg-orange-50" value="${data.qty_si || 0}" oninput="formatInput(this)"></td><td class="p-2 border-l border-blue-100"><input type="text" class="qty-so-input w-full text-center border border-blue-200 rounded p-1.5 text-xs font-bold text-blue-700 bg-blue-50" value="${data.qty_so || 0}" oninput="formatInput(this)"></td><td class="p-2"><input type="text" class="si-input w-full text-right border border-gray-200 rounded p-1.5 text-xs text-gray-600 focus:border-orange-500 outline-none" value="${fmn(data.si || 0)}" oninput="formatInput(this)" placeholder="Giá vốn"></td><td class="p-2"><input type="text" class="so-input w-full text-right border border-gray-200 rounded p-1.5 text-xs text-gray-600 font-bold focus:border-blue-500 outline-none" value="${fmn(data.so || 0)}" oninput="formatInput(this)" placeholder="Giá bán"></td><td class="p-2 text-right font-bold text-orange-600 bg-orange-50/50 text-xs total-si-row border-l border-orange-100">0</td><td class="p-2 text-right font-bold text-blue-600 bg-blue-50/50 text-xs total-so-row border-l border-blue-100">0</td><td class="p-2 text-center"><button type="button" onclick="this.closest('tr').remove(); calcAll();" class="text-gray-400 hover:text-red-500"><i class="fa-solid fa-trash"></i></button></td>`;
...
export function calcAll() {
    ...
    document.querySelectorAll('#salesDetailBody tr').forEach(tr => {
        ...
        totalImportVal += (qtySI * si); totalRev += (qtySO * so); totalCOGS_Real += (qtySO * si); totalSoldQty += qtySO;
    });
...
export function ui_renderHistoryTable(reports) {
    ...
    document.getElementById('historyBody').innerHTML = (reports || []).map(r => {
        ...
        const statusBadge = isPending ? `<span class="bg-orange-100 text-orange-600 px-2 py-1 rounded text-[10px] font-bold border border-orange-200">⏳ CHỜ DUYỆT</span>` : `<span class="bg-blue-100 text-blue-600 px-2 py-1 rounded text-[10px] font-bold border border-blue-200">✅ ĐÃ DUYỆT</span>`;
        let actions = "";
        if (isPending) {
            actions += `<button onclick="window.editReport('${r.report_id}')" class="text-blue-500 hover:bg-blue-100 p-2 rounded mx-1" title="Sửa"><i class="fa-solid fa-pen-to-square"></i></button><button onclick="window.deleteReport('${r.report_id}')" class="text-red-500 hover:bg-red-100 p-2 rounded mx-1" title="Xóa"><i class="fa-solid fa-trash"></i></button>`;
            if (isDirector) { actions += `<button onclick="window.viewReportDetail('${r.report_id}')" class="text-purple-600 hover:bg-purple-100 p-2 rounded mx-1 border border-purple-200 shadow-sm" title="Xem Chi Tiết P&L"><i class="fa-solid fa-eye"></i></button>`; }
        } else {
            if (isDirector) { actions = `<button onclick="window.viewReportDetail('${r.report_id}')" class="text-gray-500 hover:bg-gray-100 p-2 rounded mx-1" title="Xem lại"><i class="fa-solid fa-eye"></i></button>`; }
            actions += `<span class="text-gray-300 p-2"><i class="fa-solid fa-lock"></i></span>`;
        }
        const shopInfo = STATE.globalShopMap[r.shop_code] || {};
        return `<tr class="hover:bg-gray-50 border-b transition"><td class="p-4 font-bold text-xs whitespace-nowrap">${r.report_month}</td><td class="p-4 text-xs font-bold text-purple-600">${shopInfo.area || "---"}</td><td class="p-4 text-xs font-bold text-slate-800 uppercase">${shopInfo.shop_name || r.shop_code}<div class="text-[9px] text-gray-400 font-normal">${r.shop_code}</div></td><td class="p-4 text-xs font-black text-orange-600 text-center">${r.sold_quantity || 0}</td><td class="p-4 text-right font-mono font-bold text-blue-600 text-xs">${fmn(r.actual_revenue)}</td><td class="p-4 text-center">${statusBadge}</td><td class="p-4 text-center whitespace-nowrap flex justify-center items-center">${actions}</td></tr>`;
    }).join('');
}
...
export function exportDirectorExcel() {
    ...
    STATE.cachedDirectorData.forEach(r => { const k = calcKPI(r); const margin = k.rev > 0 ? (k.net / k.rev) * 100 : 0; wsDataAgg.push([r.shop_code, STATE.globalShopMap[r.shop_code]?.shop_name, r.report_month, k.rev, k.net, k.totalExp, margin.toFixed(2), r.sold_quantity, k.mkt]); });
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(wsDataAgg), "Tong_Hop"); XLSX.writeFile(wb, `Bao_Cao_Giam_Doc_${new Date().toISOString().slice(0, 10)}.xlsx`);
}
...
export function initChartFilter() {
    ...
    if (STATE.currentUser.role !== 'Giám Đốc' && STATE.currentUser.role !== 'Admin') { saleSel.value = STATE.currentUser.full_name; saleSel.disabled = true; }
...
export function updateChartFilters(level) {
    ...
    const shopOptions = dvns.map(s => `<option value="${s.shop_code}">${STATE.submittedShops.has(s.shop_code) ? "✅ " : ""}${s.shop_code} - ${s.shop_name}</option>`).join('');
