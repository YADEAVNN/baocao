...
export async function api_loadShopsAndLock(profile) {
    ...
    const myRole = profile.role ? profile.role.trim().toLowerCase() : ""; // Chuẩn hóa Role về chữ thường

    // Phân quyền linh hoạt hơn
    if (myRole === 'admin') {
        myShops = allShops;
    } else if (myRole.includes('giám đốc')) {
        // Khớp với mọi role có chữ "giám đốc" (Giám Đốc, Giám Đốc Miền, Giám Đốc Vùng...)
        myShops = allShops.filter(s => {
            // Lấy tên GĐ trong bảng master_shop_list để so sánh
            const dirName = s.director_name ? s.director_name.trim().toLowerCase() : "";
            // Bổ sung dự phòng nếu DB anh dùng tên cột khác cho GĐ Miền (như gd_mien, regional_director)
    ...
    // Nếu quản lý đúng 1 shop (như nhân viên cửa hàng) thì tự động khóa (disable) các ô chọn
    if (myShops.length === 1) {
        const s = myShops[0];
