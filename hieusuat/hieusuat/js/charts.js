...
// ==========================================
// BẮT ĐẦU KHỐI HOÀN THÀNH TIẾN ĐỘ (TARGET)
// ==========================================
...
// ==========================================
// TÍNH NĂNG: CẢNH BÁO THIẾU SỐ (MODAL & COPY ZALO)
// ==========================================

window.showMissingReportsModal = () => {
    if (!preCalculatedTargetData || preCalculatedTargetData.length === 0) {
    ...
    let displayShops = preCalculatedTargetData.filter(s => {
        if (dir && s.director_name !== dir) return false;
        if (sale && s.sale_name !== sale) return false;
        if (svn && s.svn_code !== svn) return false;
        if (searchInput) {
    ...
    let warningText = `🚨 *CẢNH BÁO KỶ LUẬT BÁO CÁO THÁNG ${monthStr}* 🚨\n\n`;
    warningText += `Kính gửi các GĐ Vùng & Sale Phụ Trách,\n`;
    warningText += `Dưới đây là danh sách các Đại lý chưa hoàn thành nộp báo cáo hàng ngày (Tính đến hiện tại):\n\n`;
    ...
    displayShops.forEach(s => {
        ...
        if (missingDates.length > 0) {
            hasMissing = true;
            warningText += `*${counter}. ${s.shop_name || 'Không tên'}* (${s.shop_code})\n`;
            warningText += `❌ Thiếu ngày: ${missingDates.join(', ')}\n\n`;
            counter++;
        }
    });

    if (!hasMissing) {
        warningText += `✅ Tuyệt vời! Tất cả các đại lý trong bộ lọc đều đã nộp báo cáo đầy đủ!\n`;
    }
...
window.copyMissingReports = () => {
    if (window.missingReportRawText) {
        navigator.clipboard.writeText(window.missingReportRawText).then(() => {
            alert("✅ Đã copy nội dung cảnh báo. Bạn có thể mở group Zalo và dán vào luôn nhé!");
        }).catch(err => {
...
// ==========================================
// BẮT ĐẦU KHỐI TAB MỚI: BẢNG XẾP HẠNG (LEADERBOARD)
// ==========================================
let isLdbBottomMode = false;
let ldbDataStore = [];

window.toggleLeaderboardMode = () => {
    isLdbBottomMode = !isLdbBottomMode;
    
    const btnTxt = document.getElementById('txtToggleLeaderboard');
    const btnIcon = document.getElementById('iconToggleLeaderboard');
    const titleRev = document.getElementById('title_ldb_rev');
    const titleRpt = document.getElementById('title_ldb_rpt');
    
    if (isLdbBottomMode) {
        btnTxt.innerText = "Xem Top 10 Tốt Nhất";
        btnIcon.className = "fa-solid fa-chevron-up";
        
        titleRev.innerHTML = `<i class="fa-solid fa-poop text-gray-400 text-xl"></i> BÁO ĐỘNG ĐỎ DOANH THU`;
        titleRev.parentElement.querySelector('.absolute').className = "absolute top-0 left-0 w-1.5 h-full bg-gray-400";
        
        titleRpt.innerHTML = `<i class="fa-solid fa-triangle-exclamation text-red-500 text-xl"></i> BÁO ĐỘNG ĐỎ KỶ LUẬT`;
        titleRpt.parentElement.querySelector('.absolute').className = "absolute top-0 left-0 w-1.5 h-full bg-red-500";
    } else {
        btnTxt.innerText = "Xem Top 10 Tệ Nhất";
        btnIcon.className = "fa-solid fa-chevron-down";
        
        titleRev.innerHTML = `<i class="fa-solid fa-trophy text-[#F97316] text-xl"></i> BẢNG VÀNG DOANH THU`;
        titleRev.parentElement.querySelector('.absolute').className = "absolute top-0 left-0 w-1.5 h-full bg-[#F97316]";

        titleRpt.innerHTML = `<i class="fa-solid fa-medal text-green-500 text-xl"></i> TOP KỶ LUẬT BÁO CÁO`;
        titleRpt.parentElement.querySelector('.absolute').className = "absolute top-0 left-0 w-1.5 h-full bg-green-500";
    }
...
export async function loadLeaderboardDashboard() {
    ...
    const month = document.getElementById('ldb_filter_month').value || new Date().toISOString().slice(0, 7);
    const y = parseInt(month.split('-')[0]); 
    const m = parseInt(month.split('-')[1]);
    const startOfMonth = `${month}-01`;
    const endOfMonth = `${month}-${new Date(y, m, 0).getDate().toString().padStart(2, '0')}`;
    ...
    try {
        ...
        (soReports || []).forEach(r => {
            if (shopAgg[r.shop_code]) {
                ...
                details.forEach(d => {
                    const price = STATE.priceMap[d.model] || 0;
                    reportRev += (d.qty_so || 0) * price;
                });
                shopAgg[r.shop_code].rev += reportRev;
            }
        });

        ldbDataStore = Object.values(shopAgg);
        renderLeaderboardUI();
...
window.loadLeaderboardDashboard = loadLeaderboardDashboard;

function renderLeaderboardUI() {
    let sortedRev, sortedRpt;
    
    if (isLdbBottomMode) {
        sortedRev = [...ldbDataStore].sort((a, b) => a.rev - b.rev).slice(0, 10);
        sortedRpt = [...ldbDataStore].sort((a, b) => a.days_reported.size - b.days_reported.size).slice(0, 10);
    } else {
        sortedRev = [...ldbDataStore].sort((a, b) => b.rev - a.rev).slice(0, 10);
        sortedRpt = [...ldbDataStore].sort((a, b) => b.days_reported.size - a.days_reported.size).slice(0, 10);
    }

    const generateHTML = (dataList, isRev) => {
        if (dataList.length === 0) return '<li class="text-center text-sm text-gray-400 italic">Chưa có dữ liệu</li>';
        
        return dataList.map((shop, index) => {
            let rankBadge = '';

            if (!isLdbBottomMode) {
                if (index === 0) rankBadge = `<span class="bg-yellow-100 text-yellow-600 px-3 py-1.5 rounded-lg text-xs font-black shadow-sm"><i class="fa-solid fa-crown mr-1"></i> TOP 1</span>`;
                else if (index === 1) rankBadge = `<span class="bg-gray-100 text-gray-500 px-3 py-1.5 rounded-lg text-xs font-black shadow-sm">TOP 2</span>`;
                else if (index === 2) rankBadge = `<span class="bg-orange-50 text-orange-400 px-3 py-1.5 rounded-lg text-xs font-black shadow-sm">TOP 3</span>`;
                else rankBadge = `<span class="bg-slate-50 text-slate-400 px-3 py-1.5 rounded-lg text-xs font-black shadow-sm">TOP ${index + 1}</span>`;
            } else {
                rankBadge = `<span class="bg-red-50 text-red-500 px-3 py-1.5 rounded-lg text-xs font-black shadow-sm"># ${index + 1} TỆ</span>`;
            }

            const colorClass = isLdbBottomMode ? 'bg-gray-400' : (isRev ? 'bg-[#F97316]' : 'bg-green-500');
            const iconClass = isRev ? 'fa-solid fa-motorcycle' : 'fa-solid fa-check-double';

            return `
                <li class="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition border border-transparent hover:border-gray-100 bg-white shadow-sm">
                    <div class="flex items-center gap-3 md:gap-4 w-2/3">
                        <div class="w-10 h-10 rounded-full ${colorClass} text-white flex items-center justify-center text-base shadow-sm shrink-0">
                            <i class="${iconClass}"></i>
                        </div>
                        <div class="overflow-hidden">
                            <p class="text-sm font-bold text-slate-800 truncate">${shop.name}</p>
                            <p class="text-[10px] font-mono text-gray-400 mt-0.5">${shop.code}</p>
                        </div>
                    </div>
    ...
    document.getElementById('list_ldb_rev').innerHTML = generateHTML(sortedRev, true);
    document.getElementById('list_ldb_rpt').innerHTML = generateHTML(sortedRpt, false);
}
