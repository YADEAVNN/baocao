...
import { salesHTML } from './views/view-sales.js';
import { mediaHTML } from './views/view-media.js';
import { costsHTML } from './views/view-costs.js';
import { historyHTML } from './views/view-history.js';
import { chartsHTML } from './views/view-charts.js';
import { targetHTML } from './views/view-target.js';
import { leaderboardHTML } from './views/view-leaderboard.js'; // IMPORT VIEW MỚI
...
export function switchView(viewId) {
    ...
    if (STATE && STATE.globalAssignedShops && STATE.globalAssignedShops.length > 0) {
        ...
        ['select_shop_so', 'select_shop_media', 'select_shop_crm'].forEach(id => {
        ...
        const mediaDateInput = document.getElementById('media_date');
        if (mediaDateInput && !mediaDateInput.value) { mediaDateInput.value = new Date().toISOString().split('T')[0]; }
        const histMonth = document.getElementById('history_month_filter');
        if (histMonth && !histMonth.value) { histMonth.value = new Date().toISOString().slice(0, 7); }
        const tgtMonth = document.getElementById('tgt_filter_month');
        if (tgtMonth && !tgtMonth.value) { tgtMonth.value = new Date().toISOString().slice(0, 7); }
    }

    ['nav-sales', 'nav-costs', 'nav-history', 'nav-charts', 'nav-media', 'nav-target', 'nav-leaderboard'].forEach(n => {
    ...
    if (viewId === 'history') { updateHistoryFilters('init'); api_loadSaleHistory(); }
    if (viewId === 'charts') { loadOverviewDashboard(); }
    if (viewId === 'target') { loadTargetDashboard('init'); }
    if (viewId === 'leaderboard' && typeof window.loadLeaderboard === 'function') { window.loadLeaderboard(); }

    if(typeof window.updateLanguage === 'function') window.updateLanguage(window.currentLang);
}
...
export function applyHistoryFilter() {
    renderFilteredSO(); renderFilteredMedia(); renderFilteredCRM();
}
...
function renderFilteredSO() {
    ...
    tbody.innerHTML = data.map(r => {
        ...
        let actions = `
            <button onclick="window.editDailySO('${r.id}')" class="text-blue-500 hover:bg-blue-100 p-2 rounded mx-1"><i class="fa-solid fa-pen-to-square"></i></button>
            <button onclick="window.deleteDailySO('${r.id}')" class="text-red-500 hover:bg-red-100 p-2 rounded mx-1"><i class="fa-solid fa-trash"></i></button>
        `;
...
function renderFilteredMedia() {
    ...
    tbody.innerHTML = data.map(r => {
        let actions = `
            <button onclick="window.editMediaReport('${r.id}')" class="text-blue-500 hover:bg-blue-100 p-2 rounded mx-1"><i class="fa-solid fa-pen-to-square"></i></button>
            <button onclick="window.sb.from('media_reports').delete().eq('id', '${r.id}').then(()=>window.loadSaleHistory())" class="text-red-500 hover:bg-red-100 p-2 rounded mx-1"><i class="fa-solid fa-trash"></i></button>
        `;
        let linkHTML = '---';
        if (r.media_link) {
...
function renderFilteredCRM() {
    ...
    tbody.innerHTML = data.map(c => {
        ...
        let actions = `
            <button onclick="window.editCRMReport('${c.id}')" class="text-blue-500 hover:bg-blue-100 p-2 rounded mx-1"><i class="fa-solid fa-pen-to-square"></i></button>
            <button onclick="window.sb.from('crm_customers').delete().eq('id', '${c.id}').then(()=>window.loadSaleHistory())" class="text-red-500 hover:bg-red-100 p-2 rounded mx-1"><i class="fa-solid fa-trash"></i></button>
        `;
        ...
        return `<tr class="border-b hover:bg-gray-50">
            ...
            <td class="p-4 text-xs text-gray-600 max-w-[200px] truncate">${noteText}</td>
